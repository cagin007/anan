...
module.exports.run = async (bot, message, args) => {
  ...
  "codef", 
        ...
        if (oyndurum.has(message.channel.id)) return message.reply('<a:elite_dnd:786178421502902293> Kanal başına sadece bir adam asmaca oyunu meydana gelebilir.');
        ...
        try {
            ...
            while (cevap.length !== confirmation.length && point < 6) {
                await message.channel.send(stripIndents`
                    ${displayText === null ? '**<a:elite_dnd:786178421502902293> Kobra Adam Asmaca**!' : displayText ? '**<a:elite_dnd:786178421502902293> Çok iyisin!**' : '**<a:elite_dnd:786178421502902293> Yanlış Harf!**'}
                         **Kelime:**    \`${display.join(' ')}\`
                ...
                if (!guess.size) {
                    await message.channel.send('<a:elite_hayir_2:777846256814653441> Zamanın doldu!');
                    break;
                }
            ...
            if (cevap.length === confirmation.length || tahmin) return message.channel.send(`**<a:elite_tik_1:777834176774144020> Tebrikler kelimeyi buldun! **${cevap}!`);
            return message.channel.send(`<a:elite_hayir_2:777846256814653441> Maalesef bilemedin kelime bu: **${cevap}**`);
        } catch (err) {
            oyndurum.delete(message.channel.id);
